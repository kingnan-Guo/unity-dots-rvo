...
namespace Nebukam.ORCA
{
    public class ORCASetup : MonoBehaviour
    {
        ...
        // 迭代世界 ORCA --> simulation ; 
        // 迭代世界 的分组：
        //  agents (游戏角色， 基于半径的障碍物的形状) :
        //  1、静态障碍物 (比较复杂 多边形顶点来生成的 ) ， 
        //  2、动态障碍物 (游戏角色 形状多边形 的顶点来生成， 船 飞机等) ;
        //  3、射线检测 ( 放射线 来做检测 ，看看到底打到了谁，  射线检测， 生成射线， 生成射线检测的障碍物) ;
        ...
        [Header("Settings")]
        public int seed = 12345; // 随机数种子
        public Transform target; 
        public AxisPair axis = AxisPair.XY;

        [Header("Agents")]
        public int agentCount = 50; // 游戏角色数量
        public float maxAgentRadius = 2f; // 游戏角色半径
        public float maxSpeed = 1f; // 游戏角色最大速度
        public float minSpeed = 1f; // 游戏角色最小速度

        [Header("Obstacles")]
        public int obstacleCount = 10;// 静态障碍物数量
        public int dynObstacleCount = 20;// 动态障碍物数量
        public float maxObstacleRadius = 2f; // 障碍物半径
        public int minObstacleEdgeCount = 2;// 障碍物最小边数
        public int maxObstacleEdgeCount = 2; // 障碍物最大边数
        ...
        {
            //Make sure to clean-up the jobs 销毁流程
